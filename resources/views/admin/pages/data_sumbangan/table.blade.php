...
<div class="modal fade" id="detailSumbanganModal" tabindex="-1" role="dialog" aria-labelledby="editedModalLabel" aria-hidden="true">
    <form id="form_multi_edit" name="form_multi_edit" enctype="multipart/form-data" method="post" onsubmit="approve_pembayaran(this); return false;">
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body card-block">
                                    ...
                                    <div class="row">
                                        ...
                                        <input type="hidden" name="t_hidden_index" id="t_hidden_index"  />
                                ...
                                <div class="card-body card-block">
                                    <input type="file" name="f_upload_news"   id="f_upload_news" accept="image/png, image/gif, image/jpeg" required /> <br />
                                    <img src="" id="img_detail_sumbangan" alt="detail sumbangan" title="detail sumbangan" class="img-fluid" />
                                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button id="btn_approve_pmb" name="btn_approve_pmb" type="submit" class="btn btn-success">Approve</button>
                </div>
...
<div class="content mt-3" id="div_form_berita" style="display:none;">
 ...
 <div class="row" id="view_berita_div">
        ...
        <div class="col-md-12" style="background:#F5F5F5;">
            ...
            <div class="row">
                <div class="col-md-3" style="padding:5px; text-align:center; box-sizing:border-box; border-right:1px solid #DDDDDD;">
                    <b> Anonim </b> <br />
                    Rp. <span id="span_anonim_total"><?php echo format_rupiah($total_anonim); ?></span> , -
                </div>
                <div class="col-md-3" style="padding:5px; text-align:center;  box-sizing:border-box; border-right:1px solid #DDDDDD;">
                <b> Donatur </b> <br />
                    Rp.  <span id="span_donatur_total"><?php echo format_rupiah($total_karyawan); ?></span> , -
                </div>
                <div class="col-md-3" style="padding:5px; text-align:center;  box-sizing:border-box; border-right:1px solid #DDDDDD;">
                <b> Investor </b> <br />
                    Rp. <span id="span_investor_total"><?php echo format_rupiah($total_usaha); ?></span> , -
                </div>
                <div class="col-md-3" style="padding:5px; text-align:center;  box-sizing:border-box; border-right:1px solid #DDDDDD;">
                <b> Keseluruhan </b> <br />
                    Rp. <span id="span_keseluruhan_total"><?php echo format_rupiah($total_all); ?></span> , -
                </div>
            </div>
        </div>
    </div>
            ...
            <table id="ikantable" class="table table-striped table-bordered display no-wrap datatable"
                ...
                <tbody>
                        ...
                        foreach($sumbangan as $rows){
                            ...
                            $check  = $rows->approved == 1 ? "<div  style='color:green;'> <i class='fa fa-check'></i> Approved </div>" : "<div  style='color:green; display:none;' id='div_approval_pmb".$rows->id_sumbangan_sukarela."'> <i class='fa fa-check'></i> Approved </div>";
                        ...
                        <tr>
                            ...
                            <td>
                                Rp. <?php echo format_rupiah($rows->nominal); ?> , - <br /><small><?php echo Config::get("myconfig.metode_pembayaran")[($rows->metode)-1]; ?></small><br /><small>Via &raquo; <b> <?php echo $rows->nama_bank_asli; ?> </b> </small>
                            </td>
                            <td>
                                <?php echo $rows->kode_invoice; ?> <br />
                                <small> <?php echo $rows->deskripsi; ?> </small>
                            </td> 
                            <td>
                                <?php echo tgl_indo($tgl[0])."<br />".$check; ?>
                            </td>
                            <td> 
                                <a href="javascript:void(0)" onclick="view_details('<?php echo $rows->bukti_pembayaran; ?>','<?php echo $nama; ?>','<?php echo tgl_indo($tgl[0]); ?>','<?php echo $rows->deskripsi; ?>','<?php echo $rows->approved; ?>','<?php echo $rows->id_sumbangan_sukarela; ?>');"> <i class="fa fa-eye"></i> Detail </a> 
                            </td>
                        </tr>
    ...
    function approve_pembayaran(form){

        var form_data = new FormData(form);

        $.ajax({
                url: url_menu_apis+"/"+"approve-pembayaran-baru", // point to server-side PHP script 
                ...
                success: function(response){
                   // alert(php_script_response); // display response from the PHP script, if any
                    //ambil_berita(response);ss
                    //openView();
                    var index_pmb = $("#t_hidden_index").val();

                    $("#div_approval_pmb"+response).show();
                    $("#detailSumbanganModal").modal("hide");
    ...
    function view_details(bukti,nama,tanggal,deskripsi,approved,index){

        $("#f_upload_news").val("");
        $("#t_hidden_index").val(index);

        $.ajax({
            type:"GET",
            data:"index="+index,
            url:url_menu_apis+"/get-sumbangan-detail/"+index,
            dataType:'json',
            success:function(data){

                if(data.approved == 0){
                    $("#img_detail_sumbangan").hide();
                    $("#f_upload_news").show();
                    $("#btn_approve_pmb").show();
                }
                else{
                    $("#img_detail_sumbangan").show();
                    $("#f_upload_news").hide();
                    $("#btn_approve_pmb").hide();
                }
                //alert("show");
                
                $("#img_detail_sumbangan").prop("src" , url_menu_asset+"/sumbangan/"+data.bukti_pembayaran);

                $("#span-nama-penyumbang").html(data.nama);
                $("#span-metode-penyumbang").html(data.nama_metode);
                $("#span-tanggal-penyumbang").html(data.tanggal);
                $("#span-deskripsi-penyumbang").html(data.deskripsi); 
    ...
    function get_total_reload(){

        $.ajax({
            type:"GET",
            data:"",
            url:url_menu_apis+"/"+"sumbangan-online-api",
            dataType:"json",
            success:function(data){
                $("#span_anonim_total").html(data.total_anonim);
                $("#span_donatur_total").html(data.total_karyawan);
                $("#span_investor_total").html(data.total_usaha);
                $("#span_keseluruhan_total").html(data.total_all);
            }
        });
