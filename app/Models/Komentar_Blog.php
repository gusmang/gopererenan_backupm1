...
class Komentar_Blog extends Model
{   
    //public $timestamps = false; 
    //
    protected $fillable = ['id_komentar_blog', 'nama_komentar', 'deskripsi','aktif'];
    protected $table='tb_komentar_blog';

    public static function get_datakomentar_cat($request,$index){

        $data = Komentar_Blog::where("id_blog",$index)->orderBy("id_komentar_blog" , "desc")->paginate();

        return $data;

    }

    public static function post_datakomentar_cat($request,$index){
        $data                            = new Komentar_Blog;

        $data->nama_komentar             = $request->t_komentar_name;
        $data->deskripsi                 = $request->t_deskripsi_name;
        $data->id_blog                   = $index;
