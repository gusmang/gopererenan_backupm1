...
class UsahaController extends BaseController
{
    ...
    public function ambil_listUsaha(Request $request){
        ...
        if(isset($nama_usaha) || isset($banjar)){
            $usaha = Usaha::get_dataUsaha_search($request);
            //echo "heres2";
        }
        else{
    ...
    public function get_detailUsaha(Request $request , $index){
        ...
        for($an = 1; $an <= 12; $an++){
            ...
            $cek_dana = Danapunia::where("id_usaha",$index)->where("tanggal_pembayaran_alt", ">=" , $awal_bln)->where("tanggal_pembayaran_alt", "<=" , $akhir_bln)->count();
            ...
            if($cek_dana > 0){
                ...
                $dana_rows = Danapunia::where("id_usaha",$index)->where("tanggal_pembayaran_alt", ">=" , $awal_bln)->where("tanggal_pembayaran_alt", "<=" , $akhir_bln)->firstOrfail();
                if($dana_rows->on_progress == "0"){
                    array_push($arr_sts_punia,"y");
                }
                else{
                    array_push($arr_sts_punia,"p");
                }
        ...
        $validasi = Danapunia::validasi_tanggal_punia();
    ...
    public function kirim_reminder(Request $request,$emails){
        // echo $emails;
        // die();
        $data_usaha = Usaha::get_detailUsaha_byEmail($request->email);

        
        try{
            Mail::to(strtolower($emails))->send(new Confirm_Withdraw_Request($data_usaha,strtolower($emails)));
    ...
    public function update_pembayaran_baru(Request $request , $index){
        $usaha = Usaha::update_pembayaran_baru($request,$index);
